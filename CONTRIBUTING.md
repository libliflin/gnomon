...
Copy `theater_violations_img_missing_dimensions_fires` in `src/audit.rs` (line 1081) as your test template.
Copy `analyze_html_img_missing_dimensions` in `src/analyze.rs` (search `img_missing_dimensions`) as your detection template.
...
Copy any test in `src/forbidden.rs` — e.g. `forbidden_gtm_matches` — as your test template.
...
Copy `render_blocking_detail_one_name` in `src/audit.rs` (search `render_blocking_detail_one_name`) as your test template.
...
`HtmlAnalysis.has_charset_meta` is detected in `analyze_html` (tested at line 730) and serialized to JSON, but `theater_violations` has no branch for it. PLAN.md §7.1 lists "missing charset in first 1024 bytes" as an anti-theater rule. The contribution is one branch and two tests — follow the `img_missing_dimensions` pattern exactly.
