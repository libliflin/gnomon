...
Copy `img_missing_both_dimensions_is_counted` in `src/analyze.rs` (line 510) as your detection template.
...
Copy any test in `src/forbidden.rs` — e.g. `multiple_distinct_entries_each_match` (line 105) — as your test template.
...
Copy `render_blocking_detail_single_name` in `src/audit.rs` (line 619) as your test template.
...
`HtmlAnalysis.has_charset_meta` is detected in `analyze_html` (tested at line 728) and serialized to JSON, but `theater_violations` has no branch for it. PLAN.md §7.1 lists "missing charset in first 1024 bytes" as an anti-theater rule. The contribution is one branch and two tests — follow the `img_missing_dimensions` pattern exactly.
